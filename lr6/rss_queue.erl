...
%% @doc handle_call - синхронная функция, клиент ждет ответ
...
%% {sub,QPid} - подписывает указанную очередь RSS на поручение сообщений очереди
%% {get_all} - запрашивает текущий список эл-ов RSS ленты. Список возвращает с ид ReqPID
...
%% @doc handle_cast асинхронный как уведомление 
%% @doc Эта функция вызывается в тот момент когда gen_server 
%% процесс получает запрос, посланный с использованием 
...
%% @doc terminate возвращает начальное состояние и атом ок 
...
%% для соответствия реалиям работающего кода
